import type { CardData } from "@/types/card";

export const federalSignetCreditCard: CardData = {
  id: "federal-signet-credit-card",
  name: "Federal Signet Credit Card",
  tagline: "Premium Visa Platinum with 3X rewards and lounge access",
  description: "The Federal Signet Credit Card is a premium Visa Platinum card offering 3 reward points per ₹100 spent. With 4 complimentary domestic lounge visits per year and a host of lifestyle benefits, it provides excellent value for professionals seeking a well-rounded credit card at a modest annual fee.",
  bank: "Federal Bank",
  bankCode: "federal",
  network: "Visa",
  cardType: "credit",
  tier: "Platinum",
  variant: "Signet",
  usage: "Domestic",
  bin: "",
  material: "Plastic",
  colorScheme: "Silver and blue",
  virtualCardAvailable: true,

  eligibility: {
    employmentType: ["Salaried", "Self-Employed"],
    minSalary: 50000,
    minAge: 21,
    maxAge: 65,
    minAnnualIncome: 600000,
    minCreditScore: 720,
    existingAccountRequired: false,
    invitationOnly: false,
    notes: [
      "Salaried: Net Monthly Income > ₹50,000",
      "Self-Employed: ITR > ₹6 Lakh p.a.",
      "Age 21–65 years",
    ],
  },

  charges: [
    { label: "Joining Fee", amount: 999, amountWithGst: 1179, note: "Plus GST" },
    { label: "Annual Fee", amount: 999, amountWithGst: 1179, note: "Plus GST, waived on ₹3L annual spend" },
    { label: "Add-on Card Fee", amount: 0, note: "Free" },
  ],

  atmCharges: {
    ownBankFreePerMonth: 5,
    ownBankCharge: 0,
    otherBankFreePerMonth: 3,
    otherBankCharge: 250,
    internationalWithdrawalFee: 250,
    internationalWithdrawalPercent: 2.5,
    balanceEnquiryOwnBank: 0,
    balanceEnquiryOtherBank: 25,
    miniStatementOtherBank: 25,
  },

  transactionCharges: {
    currencyMarkupPercent: 3.5,
    crossBorderFee: 0,
    dccFeePercent: 1,
    smsAlertPerMonth: 0,
    pinRegenerationCharge: 0,
    physicalStatementCharge: 100,
    chequeBounceCharge: 500,
    latePaymentFee: 1000,
    overLimitFee: 500,
    cashAdvanceFeePercent: 2.5,
    cashAdvanceFlatFee: 500,
    interestRatePerMonth: 3.5,
    annualInterestRate: 42,
  },

  fuelSurcharge: {
    available: true,
    waiverPercent: 1,
    minTransactionAmount: 400,
    maxTransactionAmount: 5000,
    maxWaiverPerCycle: 250,
    fuelNetworks: [],
  },

  limits: {
    atmPerDay: 50000,
    posEcomPerDay: 500000,
    contactlessPerTxn: 5000,
    contactlessDailyLimit: 50000,
  },

  insurance: {
    accidentalDeathCover: 2000000,
    permanentDisabilityCover: 2000000,
    purchaseProtectionCover: 50000,
    purchaseProtectionDays: 60,
    lostCardLiability: 500000,
    lostCardLiabilityWindow: 24,
    travelInsuranceCover: 0,
    airAccidentCover: 0,
    baggageCover: 0,
    conditions: [
      "Personal accident cover: ₹20 Lakh",
      "Lost card liability: ₹5 Lakh",
    ],
    provider: "Federal Bank",
  },

  loungeAccess: {
    domestic: {
      count: 4,
      frequency: "1 per quarter",
      frequencyPerYear: 4,
      valuePerVisit: 2000,
      guestAllowed: false,
      guestCharge: 0,
    },
    international: {
      count: 0,
      frequency: "Not available",
      frequencyPerYear: 0,
      valuePerVisit: 0,
      guestAllowed: false,
      guestCharge: 0,
    },
    programs: ["Dreamfolks"],
    conditions: [
      "4 complimentary domestic lounge visits per year (1 per quarter)",
    ],
  },

  benefits: [
    {
      category: "lounge",
      title: "Domestic Lounge Access",
      description: "4 complimentary domestic airport lounge visits per year",
      valueStr: "4 visits/year",
      valueNum: 2000,
      frequencyStr: "1 per quarter",
      frequencyPerYear: 4,
      annualValue: 8000,
      conditions: ["1 visit per quarter at domestic lounges"],
      isSellable: false,
      sellValue: 0,
      isActive: true,
      activationRequired: false,
    },
    {
      category: "rewards",
      title: "3 Reward Points per ₹100",
      description: "Earn 3 reward points on every ₹100 spent",
      valueStr: "3 RP/₹100",
      valueNum: 0.75,
      frequencyStr: "Every transaction",
      frequencyPerYear: 0,
      annualValue: 0,
      conditions: ["Excluded: fuel, rent, wallet loads"],
      isSellable: false,
      sellValue: 0,
      isActive: true,
      activationRequired: false,
    },
    {
      category: "fuel",
      title: "Fuel Surcharge Waiver",
      description: "1% fuel surcharge waiver on transactions between ₹400–₹5,000",
      valueStr: "1% waiver",
      valueNum: 250,
      frequencyStr: "Per statement cycle",
      frequencyPerYear: 12,
      annualValue: 3000,
      conditions: ["Valid on transactions ₹400–₹5,000", "Max ₹250 per statement cycle"],
      isSellable: false,
      sellValue: 0,
      isActive: true,
      activationRequired: false,
    },
  ],

  rewardProgram: {
    name: "Federal Reward Points",
    earnRate: "3 RP per ₹100 on all spends",
    pointsPer100: 3,
    pointValue: 0.25,
    effectiveCashbackPercent: 0.75,
    pointsExpiry: "Valid for 3 years",
    bonusCategories: [],
    redemptionOptions: [
      "Products & Vouchers via Federal Rewards Portal",
      "Flights & Hotels",
      "Cashback (1 RP = ₹0.25)",
    ],
    minRedemptionPoints: 500,
    rewardRateStr: "0.75%",
    conditions: ["Excluded: fuel, rent, wallet loads, EMI conversion"],
  },

  welcomeBonus: {
    available: false,
    description: "No welcome bonus currently offered",
    valueStr: "Nil",
    valueNum: 0,
    conditions: [],
  },

  milestoneBonuses: [
    {
      description: "Annual fee waiver on spending ₹3 Lakh in a year",
      spendRequired: 300000,
      spendPeriod: "Annual",
      rewardStr: "Annual fee waiver",
      rewardNum: 1179,
    },
  ],

  feeWaivers: [
    {
      description: "Annual fee waived on spending ₹3 Lakh in a membership year",
      annualSpendRequired: 300000,
      waives: "Annual Fee",
    },
  ],

  features: [
    "Visa Platinum privileges",
    "Contactless payments",
    "EMV chip security",
    "Domestic acceptance",
    "Add-on cards available",
    "EMI conversion facility",
    "Fuel surcharge waiver",
  ],

  validity: "5 years",
  contactless: true,
  ncmc: false,

  creditCardDetails: {
    billingCycleDays: 30,
    gracePeriodDays: 50,
    minimumDuePercent: 5,
    minimumDueFlat: 200,
    balanceTransfer: false,
    emiConversion: true,
    addOnCardAvailable: true,
    maxAddOnCards: 2,
    addOnCardFee: 0,
  },

  gradientColors: ["#c0c0c0", "#1565c0"],
  value: {
    highestValue: 15000,
    averageValue: 12000,
    isSellable: false,
    marketPrice: { minSellPrice: 0, maxSellPrice: 0, averageMarketValue: 0 },
    annualNetValue: 10821,
    tenYearNetValue: 108210,
    totalAnnualCharges: 1179,
    totalAnnualBenefits: 12000,
    roiPercent: 1019,
  },

  emiOptions: {
    available: true,
    minTransactionAmount: 2500,
    tenureOptions: [3, 6, 9, 12],
    interestRatePerMonth: 1.5,
    processingFeePercent: 1,
  },

  category: "rewards",
  internationalUsable: false,
  bestFor: ["Rewards", "Lounge Access", "Fuel Savings"],

  dataQuality: {
    status: "verified",
    verifiedFields: ["charges", "rewards", "loungeAccess"],
    unverifiedFields: ["insurance"],
    sourceUrls: [
      "https://www.federalbank.co.in/credit-cards/signet-credit-card",
    ],
    lastVerified: "2026-03-27",
  },

  customerCareNumber: "1800 420 1199",
  applyUrl: "https://www.federalbank.co.in/credit-cards/signet-credit-card",
  lastUpdated: "2026-03-27",
};
